...
void ui_version(const char *ver) {
  ...
  printf("  " UI_DIM "Target  :" UI_RESET "  " UI_WHITE "C17 (Strict Safe Output)" UI_RESET "\n");
...
void ui_help(const char *prog) {
  ...
  printf("    " UI_GREEN "build" UI_RESET "        Compile, verify, and emit C17 / binary\n");
  ...
  printf("    " UI_CYAN "-o" UI_RESET " " UI_DIM "<path>" UI_RESET
         "     Output directory or binary name\n");
  ...
  printf("    " UI_CYAN "--time" UI_RESET "        Measure individual compiler passes\n");
...
void ui_success(const char *msg) {
  printf(UI_TAG_SUCCESS UI_GREEN "%s" UI_RESET "\n", msg);
}

void ui_error(const char *msg) {
  fprintf(stderr, UI_TAG_ERROR UI_RED "%s" UI_RESET "\n", msg);
}
